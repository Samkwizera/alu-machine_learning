#!/usr/bin/env python3
"""A complete bidirectional recurrent neural network cell."""
...
class BidirectionalCell:
    ...
    def output(self, H):
        """Calculate the outputs for every time step."""
        output = np.matmul(H, self.Wy) + self.by
        output -= np.max(output, axis=2, keepdims=True)
        exp_output = np.exp(output)
        return exp_output / np.sum(exp_output, axis=2, keepdims=True)
